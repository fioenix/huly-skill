...
| 7 | Hợp đồng lỗi cho máy đọc | `{status, error, code, retryable}` + `hint` cho lỗi auth; 5 code; message người đọc giữ nguyên tiếng Việt | failure document `{code, message, retryable, hint, details}` + exit status theo lớp lỗi | **Ngang** (từ 18/08/2026) — họ còn exit status theo lớp lỗi, ta chưa |
...
**Tổng: ta thắng 9, họ thắng 8, ngang/chia 5** (trước 18/08/2026: 9–10–3). Nhưng đếm phiếu là cách đọc bảng
