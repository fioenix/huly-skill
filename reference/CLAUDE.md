...
Quan trọng: Sử dụng `pnpm` thay vì `npm` cho tất cả package management operations.
- Dùng `pnpm install` thay vì `npm install`
- Dùng `pnpm add <package>` thay vì `npm install <package>`
- Dùng `pnpm run <script>` thay vì `npm run <script>`
