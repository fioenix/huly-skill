...
export function documentsCommand() {
    ...
    cmd.command('list')
        ...
        try {
            await withClient(async (client) => {
                const teamspaces = await client.getTeamspaces();
                const ts = teamspaces.find((t) => t.name === teamspaceInput || t._id === teamspaceInput);
                if (!ts) {
    ...
    cmd.command('create')
        ...
        try {
            await withClient(async (client) => {
                const teamspaces = await client.getTeamspaces();
                const ts = teamspaces.find((t) => t.name === options.teamspace || t._id === options.teamspace);
                if (!ts) {
            ...
            if (isJsonMode())
                ...
                console.error(`Loi khi tao tai lieu: ${e.message}`);
            process.exitCode = 1;
        }
    });
    cmd.command('create-teamspace')
        ...
        try {
            ...
            if (isJsonMode())
                ...
                console.error(`Loi khi tao teamspace: ${e.message}`);
            process.exitCode = 1;
        }
    });
    cmd.command('read')
        ...
        try {
            await withClient(async (client) => {
                ...
                const doc = docs.find((d) => d.title?.toLowerCase().includes(titleLower) || d._id === titleInput);
                if (!doc) {
